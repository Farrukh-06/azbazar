...
export default function ProductCard({ product }: { product: AnyProduct }) {
  ...
  const title = product.nameAz || product.name || '';
  const sellerName = product.seller || product.seller_name || '';
  const reviewCount = product.reviews || product.review_count || 0;
  const discount = product.compare_price ? Math.round((1 - product.price / product.compare_price) * 100) : product.oldPrice ? Math.round((1 - product.price / product.oldPrice) * 100) : 0;
    ...
    <div className="card group overflow-hidden flex flex-col hover:shadow-md transition-shadow">
      ...
      <div className="p-3 flex flex-col flex-1">
        <Link href={`/products/${product.id}`}>
          <p className="text-xs text-[#6B46C1] mb-0.5 font-medium">{sellerName}</p>
          <h3 className="text-sm font-semibold text-gray-900 leading-snug mb-1 line-clamp-2 hover:text-[#6B46C1] transition-colors">{title}</h3>
        </Link>
        <div className="flex items-center gap-1 mb-2">
          <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
          <span className="text-xs text-gray-400">{product.rating} ({reviewCount})</span>
        </div>
        <div className="mt-auto flex items-center justify-between">
          <div>
            <span className="text-base font-bold text-gray-900">₼{product.price}</span>
            {(product.oldPrice || product.compare_price) && (
              <span className="text-xs text-gray-400 line-through ml-1.5">₼{product.oldPrice || product.compare_price}</span>
            )}
          </div>
